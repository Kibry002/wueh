...
// Mapping of strategy keys to contract types
...
// Mapping of strategy keys to display labels
...
function updateButtons(symbol) {
  ...
  if (buyBtn) {
    buyBtn.textContent = price !== undefined ? ` ${price}` : " 0";
  }

  if (runBtn) {
    runBtn.textContent = lastDigit !== undefined ? ` ${lastDigit}` : " 0";
    console.log(`updateButtons(${symbol}): runBtn set to "${runBtn.textContent}", lastDigit=${lastDigit}`); // Debug
  }
}
...
function connectWS() {
  ...
  ws.onmessage = event => {
    ...
    if (data.msg_type === "authorize") {
      ...
      const newLastDigit = getLastDigit(symbol, quote);
      console.log(`Tick received for ${symbol}: quote=${quote}, lastDigit=${newLastDigit}`); // Debug
      marketData[symbol].price = quote;
      marketData[symbol].lastDigit = newLastDigit;
      tickSubs[symbol] = id;
      ...
      if (now - lastUpdateTime >= UPDATE_DEBOUNCE_MS) {
        ...
        console.log(`Debounced update for ${symbol}, lastDigit=${newLastDigit}`); // Debug
      }
    } else if (data.msg_type === "proposal" && data.proposal) {
      const { id, ask_price, contract_type } = data.proposal;
      const contract_id = `proposal_${id}`;

      // Store proposal state
      proposalStates[id] = { status: 'success', timestamp: Date.now() };
      ...
      pendingTrades[contract_id].buySent = true;
    } else if (data.msg_type === "buy" && data.buy) {
      const { contract_id, buy_price, proposal_id } = data.buy;
      const proposal_key = `proposal_${proposal_id}`;

      // Mark proposal as bought
      if (proposal_id) {
        proposalStates[proposal_id] = { status: 'bought', timestamp: Date.now() };
      }

      if (pendingTrades[proposal_key]) {
        pendingTrades[proposal_key].buySuccess = true;
      }
      const market = pendingTrades[proposal_key]?.market || currentSymbol;
      const contract_type = pendingTrades[proposal_key]?.contract_type || pendingTrades.lastType || "Unknown";
      const strategy = pendingTrades[proposal_key]?.strategy || pendingTrades.lastStrategy || "unknown";
      const stake = pendingTrades[proposal_key]?.stake || parseFloat($("stake")?.value) || 5.00;
      const barrier = pendingTrades[proposal_key]?.barrier || "";
      let displayType = STRATEGY_LABELS[strategy] || CONTRACT_TYPE_LABELS[contract_type] || contract_type;
      let infoMsg = `Trade executed: ${displayType}` +
        (barrier ? ` barrier=${barrier}` : "") +
        ` @ $${stake.toFixed(2)} for ${SYMBOL_DISPLAY_NAMES[market] || market}`;
      showTradeToast({
        type: "executed",
        msg: infoMsg,
        contract_id,
      ...
      delete pendingTrades[proposal_key];
...
function handleBuyButton() {
  ...
  if (buyBtn) {
    buyBtn.addEventListener('click', () => {
      if (marketData[currentSymbol]?.price !== undefined) {
        updateButtons(currentSymbol);
      } else {
        buyBtn.textContent = " N/A";
        console.log(`handleBuyButton: No price for ${currentSymbol}, setting buyBtn to N/A`); // Debug
      }
    });
  }
}
...
function handleRunButton() {
  ...
  if (runBtn) {
    runBtn.addEventListener('click', () => {
      const lastDigit = marketData[currentSymbol]?.lastDigit;
      if (lastDigit !== undefined) {
        runBtn.textContent = ` ${lastDigit}`;
        console.log(`handleRunButton: Set runBtn to "${lastDigit}" for ${currentSymbol}`); // Debug
      } else {
        runBtn.textContent = " N/A";
        console.log(`handleRunButton: No lastDigit for ${currentSymbol}, setting runBtn to N/A`); // Debug
      }
    });
  }
}
...
function validatePrediction(input, strategyKey) {
  if (!input) return;
  if (['higher', 'lower'].includes(strategyKey)) {
...
function renderForm() {
  ...
  if (strategy) {
    const isBarrier = ['higher', 'lower'].includes(strategyKey);
    const inputLabel = isBarrier ? 'Barrier' : 'Prediction';
    ...
    const placeholder = strategy.placeholder || (isBarrier ? '+ or -' : strategy.label + (strategy.readonly ? '' : ' (0-9)'));
    const value = strategy.readonly ? strategy.prediction : '';

    formHTML += `
      <div class="form-group">
        <label for="prediction">${inputLabel}</label>
        <input type="${predictionType}" class="form-input" id="prediction"
               placeholder="${placeholder}" ${readonly}
               ${minAttr} ${maxAttr} value="${value}"
               oninput="validatePrediction(this, '${strategyKey}')">
      </div>
    `;
  } else {
    formHTML += `
...
function handleTradeButton() {
  const formsContainer = $("forms-container");
  const mainStrategy = $("main-strategy");
  if (!formsContainer || !mainStrategy) return;

  let currentStrategy = mainStrategy.value;

  // Event listener to track main-strategy changes
  mainStrategy.addEventListener('change', () => {
    currentStrategy = mainStrategy.value;
  });
  ...
  formsContainer.addEventListener('click', (event) => {
    if (event.target.classList.contains('trade-button')) {
      ...
      if (!strategy.readonly) {
        if (['higher', 'lower'].includes(currentStrategy)) {
          if (!prediction || !/^[+-]?\d+(\.\d+)?$/.test(prediction)) {
            showTradeToast({
              type: "error",
              msg: "Please enter a valid barrier (e.g., +0.01, -0.01, 1, -1)",
              duration: 5000
            });
            return;
          }
        } else if (['over', 'under', 'matches', 'differs'].includes(currentStrategy)) {
          if (!prediction || isNaN(parseInt(prediction)) || parseInt(prediction) < 0 || parseInt(prediction) > 9) {
            showTradeToast({
              type: "error",
              msg: "Please enter a valid digit (0-9)",
              duration: 5000
            });
            return;
          }
      ...
      const proposal = {
        ...
        contract_type: CONTRACT_TYPE_MAP[currentStrategy],
        amount: stake,
        currency: "USD",
        duration: ticks,
        duration_unit: "t",
      ...
      // Only add barrier for compatible contract types
      if (!strategy.readonly && prediction && ['over', 'under', 'matches', 'differs', 'higher', 'lower'].includes(currentStrategy)) {
        proposal.barrier = prediction;
      }

      const contract_id = `pending_${Date.now()}`;
      pendingTrades[contract_id] = {
        market: currentSymbol,
        type: CONTRACT_TYPE_MAP[currentStrategy],
        strategy: currentStrategy,
        stake: stake,
        barrier: prediction || ""
      };
      pendingTrades.lastMarket = currentSymbol;
      pendingTrades.lastType = CONTRACT_TYPE_MAP[currentStrategy];
      pendingTrades.lastStrategy = currentStrategy;
